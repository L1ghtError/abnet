...
#ifndef ABNET_IMPL_ERROR_CODE_IPP
#define ABNET_IMPL_ERROR_CODE_IPP
...
#include "abnet/local_free_on_block_exit.hpp"
...
public:
  const char *name() const noexcept { return "abnet.system"; }

  std::string message(int value) const {
#if defined(ABNET_WINDOWS_RUNTIME) || defined(ABNET_WINDOWS_APP)
    std::wstring wmsg(128, wchar_t());
    for (;;) {
      DWORD wlength =
          ::FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, 0, value,
                           MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), &wmsg[0], static_cast<DWORD>(wmsg.size()), 0);
      ...
      if (wlength) {
        ...
        int length = ::WideCharToMultiByte(CP_ACP, 0, wmsg.c_str(), static_cast<int>(wlength), &msg[0],
                                           static_cast<int>(wlength * 2), 0, 0);
        if (length <= 0)
          return "abnet.system error";
    ...
    if (length)
      ...
      return "abnet.system error";
#else // defined(ABNET_WINDOWS_DESKTOP) || defined(__CYGWIN__)
#if !defined(__sun)
    if (value == ECANCELED)
...
#endif
#endif // defined(ABNET_WINDOWS_DESKTOP) || defined(__CYGWIN__)
  }

#if defined(ABNET_HAS_STD_ERROR_CODE)
...
#endif // defined(ABNET_HAS_STD_ERROR_CODE)
  ...
  const error_category &system_category() {
    static class system_category instance;
    return instance;
  }
...
#endif // ABNET_IMPL_ERROR_CODE_IPP